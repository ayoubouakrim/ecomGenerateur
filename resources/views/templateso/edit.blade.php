...
{{--        --}}{{--
    ...
    <div class="ai-assistant">
        ...
        <div class="ai-chat" id="aiChat">
            <div class="chat-messages" id="chatMessages"></div>
            <div class="chat-input">
                <input type="text" id="aiInput" class="form-control"
...
//
    ...
    const DEEPSEEK_API_KEY = 'sk-d73eea9363ab43fb9f0b3a3a95f86ee1'; // À remplacer
    ...
    // Gestion de l'interface IA
    document.getElementById('aiButton').addEventListener('click', () => {
        document.getElementById('aiChat').style.display =
            document.getElementById('aiChat').style.display === 'block' ? 'none' : 'block';
    });

    // Envoi des requêtes à l'IA
    document.getElementById('sendButton').addEventListener('click', async () => {
        if (aiState.isProcessing) return;
    ...
    function extractHtmlFromResponse(response) {
        // Utiliser une expression régulière pour extraire le HTML
        const htmlMatch = response.match(/```html\n([\s\S]*?)\n```/);
        return htmlMatch ? htmlMatch[1] : response;
    }
