...
<div class="main">
    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route('inputUser') }}">
        ...
        <div class="exp-cvc">
            <div class="expiration">
                <label >Logo</label>
                <input class="inputCard" name="logoUrl" type="file"/>
                <br>
            </div>
            <div class="security">
                <label for="cvc">Icon</label>
        ...
        <textarea class="textareai" name="description"  >
...
<script>
    // Fonction pour afficher la couleur sélectionnée
    document.querySelectorAll('input[type="color"]').forEach((input) => {
        input.addEventListener('input', (event) => {
            // Mettre à jour l'arrière-plan de l'input pour refléter la couleur sélectionnée
            input.style.backgroundColor = event.target.value;

            // Optionnel : afficher la couleur dans une zone de texte à côté
            const colorDisplay = input.nextElementSibling;
            if (colorDisplay && colorDisplay.classList.contains('color-display')) {
                colorDisplay.textContent = event.target.value;
            }
        });
    });
</script>
